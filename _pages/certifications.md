---
layout: archive
title: "Certifications & Awards"
permalink: /certifications/
author_profile: true
---

## Professional Certifications

### IBM / Cognitive Class

**[Data Analysis with Python](https://courses.cognitiveclass.ai/certificates/913e515ec95d4a20a111fcdd9b61556c)** (DA0101EN)  
*IBM / Cognitive Class | August 9, 2024*  
Gained practical skills in exploratory data analysis, data manipulation, and visualization using Python.

**[SQL and Relational Databases 101](https://courses.cognitiveclass.ai/certificates/bbaad986de6e484eb300bd30d5d23a86)** (DB0101EN)  
*IBM Skills Network / Cognitive Class | June 17, 2024*  
Mastered SQL fundamentals and relational database design principles for data querying and management.

### Online Certifications (Coursera)

**[Machine Learning Specialization](link coming soon)**  
*Coursera | 2024*  
Completed 3-course specialization covering supervised learning, advanced algorithms, and unsupervised learning techniques.

**[Google Data Analytics Professional Certificate](link coming soon)**  
*Coursera | 2024*  
Developed proficiency in data analysis with R, data visualization, and data cleaning methodologies.

**[Google Advanced Data Analytics Professional Certificate](link coming soon)**  
*Coursera | 2024*  
Advanced training in machine learning with Python, predictive modeling, and statistical analysis techniques.

**[Meta Data Analyst Professional Certificate](link coming soon)**  
*Coursera | 2024*  
Learned statistical analysis, Tableau visualization, and SQL database fundamentals for business analytics.

---

## Training Programs & Bootcamps

### UNESCO UNITWIN Data Science Camp

![Data Science Camp Certificate](/images/certificates/technical-certificates/data_science_camp.jpg)

**Data Science Camp — Standard Level**  
*Handong Global University, UNESCO UNITWIN | February 2, 2024*  
Completed intensive 2-week bootcamp with hands-on projects using real-world datasets and industry best practices.

### Employment Skills & Professional Development

![Employment and Soft Skills Certificate](/images/certificates/technical-certificates/basis_employment_and_soft_skills.jpg)

**Basic Employment Skill and Soft Skills**  
*Cambodian Youth Network Association | 2024*  
6-day training program covering administration, report writing, youth leadership, and professional communication.

### Digital Transformation in Engineering

![Digital Transformation Certificate](/images/certificates/technical-certificates/training_digital_tranformation_and_engineering.jpg)

**Digital Transformation in Engineering**  
*Board of Engineers, Cambodia | March 29, 2024*  
Explored emerging digital technologies and their applications in modern engineering practices.

---

## Workshops & Seminars

### Mathematics Workshop

![Mathematics Workshop Certificate](/images/certificates/technical-certificates/mathematic_workshop.jpg)

**BiM Workshop 2024: Basic in Mathematics**  
*Mathematical Association of Cambodia | November 9-10, 2024*  
In-depth workshop on Linear Algebra, Real Analysis, and Topology concepts taught by PhD-level instructors.

---

## Internship Programs

### Data Science Internship

![Data Science Internship Certificate](/images/certificates/technical-certificates/certificate_of_internshp_MLVT.jpg)

**Data Science Internship Certificate**  
*General Secretariat of National Council for Minimum Wage | Aug 4 – Oct 4, 2025*  
2-month structured program in applied data science with hands-on ETL pipeline development and data analysis projects.

---

## Volunteer & Community Service

### ASEAN Conference Volunteer

![ASEAN Conference Certificate](/images/certificates/volunteer-certificates/ASEAN_conference.jpg)

**CAFEO-40 Conference Volunteer**  
*Board of Engineers Cambodia | December 5-7, 2022*  
Recognized for dedicated service during the 40th ASEAN Federation of Engineering Organisations conference on Industry 4.0.

### Defense Day Volunteer

![Defense Day Volunteer Certificate](/images/certificates/volunteer-certificates/help_department.jpg)

**Defense Day Student Volunteer**  
*Institute of Technology of Cambodia, Department of AMS | July 9-10, 2025*  
Contributed as a volunteer to the successful execution of the ITC Defense Day academic event.

### Mathematics Tutoring

![Mathematics Tutor Certificate](/images/certificates/volunteer-certificates/teaching_math.jpg)

**Mathematics Tutor**  
*Institute of Technology of Cambodia | 2022-2023*  
Provided foundational mathematics instruction to junior students in the Foundation Year program.

---

## Youth & Competition Programs

### E-Gen Competition

![E-Gen Competition Certificate](/images/certificates/technical-certificates/EGEN_competition.jpg)

**E-Gen Competition Program 2025**  
*Directorate General of Youth, Ministry of Education | 2025*  
Participated in the E-Gen youth development program promoting digital skills and entrepreneurship.

---

### **Additional Volunteer Certificates**

![Volunteer Certificate 4](/images/certificates/volunteer-certificates/volunteer-cert-4.png)

![Volunteer Certificate 5](/images/certificates/volunteer-certificates/volunteer-cert-5.png)

---

## How to Add Certificate Images

### **Folder Structure**
Create these folders in your `images/` directory:

```
images/
├── certificates/
│   ├── volunteer-certificates/
│   │   ├── digital-literacy-trainer.png
│   │   ├── math-tutor.png
│   │   ├── volunteer-cert-4.png
│   │   └── volunteer-cert-5.png
│   ├── technical-certificates/
│   └── awards/
│       └── environment-award.png
```

### **Steps to Add Your Certificates**

1. **Create folders:**
   ```
   📁 images/certificates/volunteer-certificates/
   📁 images/certificates/awards/
   ```

2. **Save certificate images** (PNG or JPG, recommended size: 600x400px or larger)

3. **Reference in markdown:**
   ```markdown
   ![Certificate Name](/images/certificates/folder-name/filename.png)
   ```

### **Recommended Image Specifications**
- **Format:** PNG or JPG
- **Size:** 600x400px minimum (800x600px recommended)
- **Quality:** High resolution (300+ DPI for prints)
- **Naming:** Use descriptive names with hyphens (e.g., `digital-literacy-trainer.png`)

---

**Last Updated:** {{ site.time | date: '%B %d, %Y' }}
